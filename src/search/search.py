...
class UserSearch:
    ...
    async def search(self, criteria: SearchCriteria, on_page_scraped: Optional[Callable[[List[Profile]], Awaitable[None]]] = None) -> SearchResult:
        ...
        while pages_scraped < self.max_pages:
            ...
            page_profiles = await self.parser.parse_search_results()
            ...
            # Check for next page
            has_next = await self.pagination.has_next_page()
            if not has_next:
    ...
    async def _scroll_to_load_results(self) -> None:
        """Scroll through the page to load all lazy-loaded results."""
        scroll_count = 0
        max_scrolls = 5
        
        while scroll_count < max_scrolls:
            await self.browser.scroll("down", 400)
            await self.browser.humanizer.random_delay(5000, 10000)
            scroll_count += 1
...
async def search_by_keywords(
    ...
    return await searcher.search(criteria)
